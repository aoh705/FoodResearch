...
result
```
Wilcoxon rank-sum Test using ratings:
- woorijip
```{r}
w_year <- 2021
...
before_data$rating <- as.numeric(before_data$rating)
after_data$rating <- as.numeric(after_data$rating)

result <- wilcox.test(before_data$rating, after_data$rating)
result
```
- ayada
```{r}
